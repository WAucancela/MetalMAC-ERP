/**
 * /contabilidad/cuentas-por-pagar — facturas de compra con saldo pendiente
 */
...
import { useCuentasPorPagar, useRegistrarPagoCompra, type CuentaPorPagar } from '@/hooks/useCuentasPorPagar';
import type { RegistrarPagoInput } from '@/lib/validations/finanzas.schema';

function FilaCuentaPorPagar({ cuenta }: { cuenta: CuentaPorPagar }) {
  const [abierto, setAbierto] = useState(false);
  const registrar = useRegistrarPagoCompra(cuenta.id);
  ...
  return (
    <>
      <TableRow>
        ...
        <TableCell className="text-sm">{cuenta.proveedorNombre ?? '—'}</TableCell>
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <p className="text-sm text-muted-foreground">No hay facturas de compra con saldo pendiente.</p>
      ) : (
        <>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {cuentas.map((cuenta) => (
                <FilaCuentaPorPagar key={cuenta.id} cuenta={cuenta} />
              ))}
            </TableBody>
          </Table>
